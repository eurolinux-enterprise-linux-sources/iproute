...
if [ -n "$repo" -a -n "$local" ]; then
	git clone --reference $local $repo iproute >/dev/null || die "Unable to clone using local cache";
	...
	if [ -e "$local/sources" ]; then
		for i in $(cat "$local/sources"); do
			if [ -f "$local/$i" ]; then
				cp "$local/$i" "$tmp/iproute/";
			fi
		done
	fi
else
	echo "No local repo, cloning using rhpkg" >&2;
	rhpkg clone iproute >/dev/null || die "Unable to clone using rhpkg";
fi
